...
def generate_node(state: RFQState) -> dict:
    """RFQ 초안 생성 노드 — LLM은 거래조건만, 나머지는 f-string 조립 (B안)."""
    req = state["purchase_request"]
...
graph.add_edge("analyze", "search")       # ← 추가: 분석 끝나면 검색으로
...
if __name__ == "__main__":
    result = app.invoke({ 
        "purchase_request": {"item": "볼트","quantity": "100", "due_date": "2026-07-30"}
    })

    print("=== 그릇에 든 칸들 ===")
    print("키 목록:", list(result.keys()))   # rfq_draft 가 없는 거 눈으로 확인

    print("누락 항목:", result["missing_fields"])

    print("\n=== 검색된 조항 ===")
    for i, doc in enumerate(result["retrieved_docs"], 1): # 딕셔너리 안에 있는 retrieved_docs라는 키(Key)의 값(Value)이 리스트(List) 형태로 들어가 있는 것
        article = doc.metadata.get("article", "조항 미상")
        print(f"[{i}] {article}")
